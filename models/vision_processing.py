...
def create_object_mesh(image_input, depth_threshold=0.75, target_size=256):
    """
    ...
    try:
        # Apply refined background removal
        ...
        # Use result_rgb for depth estimation
...
def combine_front_back_meshes(front_fig, back_fig, depth_threshold=0.25, sampling_rate=0.5):
    ...
    # Normalize z-coordinates with safety checks
    front_z_min = np.min(front_z)
    front_z_max = np.max(front_z)
    back_z_min = np.min(back_z)
    back_z_max = np.max(back_z)
    
    # Handle case where min and max are equal
    if front_z_max == front_z_min:
        front_z_norm = np.full_like(front_z, 0.5)
    else:
        front_z_norm = 0.5 + (front_z - front_z_min) * depth_threshold / (front_z_max - front_z_min)
        
    if back_z_max == back_z_min:
        back_z_norm = np.full_like(back_z, 0.5)
    else:
        back_z_norm = 0.5 - (back_z - back_z_min) * depth_threshold / (back_z_max - back_z_min)
...
def process_orthogonal_views(front, back=None, top=None, depth_threshold=0.25, sampling_rate=1.0, target_size=256):
    """
    ...
    try:
        ...
        normalized_images, _ = load_and_normalize_images(image_paths, target_size)
        
        # Create meshes for each view
        front_fig = create_object_mesh(normalized_images['front'], depth_threshold=depth_threshold, target_size=target_size)
        back_fig = None
        if back is not None:
            back_fig = create_object_mesh(normalized_images['back'], depth_threshold=depth_threshold, target_size=target_size)
